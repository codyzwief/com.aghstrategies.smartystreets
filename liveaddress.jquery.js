...
*	ENTRY POINT
	*/
	...
	cj.LiveAddress = function(arg)
	{
		return cj(defaultSelector).LiveAddress(arg);
	};

	cj.fn.LiveAddress = function(arg)
	{
		if (instance)
			return instance;
		...
		var vers = cj.fn.jquery.split(".");
		if (vers.length >= 2)
		{
			if (vers[0] < 1 || (vers[0] == 1 && vers[1] < 5))
			{
				console.log("jQuery version "+cj.fn.jquery+" found, but LiveAddress requires jQuery version 1.5 or higher. Aborting.");
				return false;
			}
		}
		else
		...
		if (document.readyState === "complete")
			...
			cj(window).load(function() { window.loaded = true; });
	...
	*/
	function UI()
	{
		...
		this.postMappingOperations = function()
		{
			...
			if (config.ui)
			{
				...
				for (var i = 0; i < addresses.length; i++)
				{
					...
					cj('body').append('<div class="smarty-ui"><div title="Loading..." class="smarty-dots smarty-addr-'+id+'"></div></div>');
					var offset = uiTagOffset(addresses[i].corners(true));
					cj('body').append('<div class="smarty-ui" style="top: '+offset.top+'px; left: '+offset.left+'px;"><a href="javascript:" class="smarty-tag smarty-tag-grayed smarty-addr-'+id+'" title="Address not verified. Click to verify." data-addressid="'+id+'"><span class="smarty-tag-check">&#10003;</span><span class="smarty-tag-text">Verify</span></a></div>');

					// Move the UI elements around when browser window is resized
					cj(window).resize({ addr: addresses[i] }, function(e)
					{
						var addr = e.data.addr;
						var offset = uiTagOffset(addr.corners(true));	// Position of lil' tag
						cj('.smarty-tag.smarty-addr-'+addr.id())
							.parent('.smarty-ui')
							.css('top', offset.top+'px')
							.css('left', offset.left+'px');

						var addrOffset = addr.corners();				// Position of any popup windows
						cj('.smarty-popup.smarty-addr-'+addr.id())
							.parent('.smarty-ui')
						...
						{
							var containerUi = cj('.smarty-autocomplete.smarty-addr-'+addr.id()).closest('.smarty-ui');
							var domFields = addr.getDomFields();
							if (domFields['street'])
							{
								containerUi.css({
									"left": cj(domFields['street']).offset().left + "px",
									"top": (cj(domFields['street']).offset().top + cj(domFields['street']).outerHeight()) + "px"
								});
							}
						}
					});
					...
					{
						var uiTag = cj('.smarty-ui .smarty-tag.smarty-addr-'+id)
						if (uiTag.is(':visible'))
							uiTag.hide();
				...
				cj('body').delegate('.smarty-tag-grayed', 'click', function(e)
				{
					// "Verify" clicked -- manually invoke verification
					var addrId = cj(this).data('addressid');
					instance.verify(addrId);
				});

				cj('body').delegate('.smarty-undo', 'click', function(e)
				{
					// "Undo" clicked -- replace field values with previous input
					var addrId = cj(this).parent().data('addressid');
				...
				if (config.autocomplete && config.key)
				{
					...
					for (var i = 0; i < forms.length; i++)
					{
						...
						for (var j = 0; j < f.addresses.length; j++)
						{
							...
							if (domFields['street'])
							{
								var strField = cj(domFields['street']);
								var containerUi = cj('<div class="smarty-ui"></div>');
								var autoUi = cj('<div class="smarty-autocomplete"></div>');
								...
								containerUi.delegate(".smarty-suggestion", "click", { addr: addr, containerUi: containerUi }, function(event) {
									var sugg = autocompleteResponse.suggestions[cj(this).data('suggIndex')];
								...
								containerUi.delegate(".smarty-suggestion", "mouseover", function() {
									cj('.smarty-active-suggestion').removeClass('smarty-active-suggestion');
									cj(this).addClass('smarty-active-suggestion');
								});

								containerUi.delegate(".smarty-active-suggestion", "mouseleave", function() {
									cj(this).removeClass('smarty-active-suggestion');
								});
								...
								strField.keydown({ containerUi: containerUi, addr: addr }, function(event) {
									var suggContainer = cj('.smarty-autocomplete', event.data.containerUi);
									var currentChoice = cj('.smarty-active-suggestion:visible', suggContainer).first();
									...
									if (event.keyCode == 9)			// Tab key
									{
										if (currentChoice.length > 0)
										{
											...
											if (domFields['zipcode'])
												cj(domFields['zipcode']).focus();
											else
												cj(domFields['street']).blur();
									...
									else if (event.keyCode == 40)	// Down arrow
									{
										...
										{
											currentChoice = cj('.smarty-suggestion', suggContainer).first().mouseover();
									...
									else if (event.keyCode == 38)	// Up arrow
									{
										if (!currentChoice.hasClass('smarty-suggestion'))
										{
											currentChoice = cj('.smarty-suggestion', suggContainer).last().mouseover();
											choiceSelectionIsNew = true;
										}
						...
						cj(document).keyup(function(event) {
							if (event.keyCode == 27)	// Esc key
								cj('.smarty-autocomplete').closest('.smarty-ui').hide();
						});
					}

					// Try .5 and 1.5 seconds after the DOM loads to re-position UI elements; hack for Firefox.
					setTimeout(function() { cj(window).resize(); }, 500);
					setTimeout(function() { cj(window).resize(); }, 1500);
				}
			}
			...
			if (config.submitVerify)
			{
				...
				for (var i = 0; i < forms.length; i++)
				{
					...
					submitHandler = function(e)
					{
						...
						if ((e.data.form && e.data.form.processing) || cj('.smarty-active-suggestion:visible').length > 0)
							return suppress(e);
						...
						/*
							...
							"change" event on that element. Example: cj('#city').val('New City').change();
						*/
					...
					var bindSubmitHandler = function(domElement, eventName)
					{
						...
						var oldHandlers = [], eventsRef = cj._data(domElement, 'events');
						...
						if (eventsRef && eventsRef[eventName] && eventsRef[eventName].length > 0)
						{
							...
							oldHandlers = cj.extend(true, [], eventsRef[eventName]);
						}

						// Unbind them...
						cj(domElement).unbind(eventName);

						// ... then bind ours first ...
						cj(domElement)[eventName]({ form: f, invoke: domElement, invokeFn: eventName }, submitHandler);
		...
		function doAutocomplete(event)
		{
			...
			var input = cj.trim(event.data.streetField.val());
			var containerUi = event.data.containerUi;
			var suggContainer = cj('.smarty-autocomplete', containerUi);
			...
			{
				if (cj('.smarty-active-suggestion:visible').length > 0)
					useAutocompleteSuggestion(addr, autocompleteResponse.suggestions[cj('.smarty-active-suggestion:visible').first().data('suggIndex')], containerUi);
				containerUi.hide();
				streetField.blur();
		...
		this.requestAutocomplete = function(event, data)
		{
			...
			var autocplrequest = {
				callback: function(counter, json)
				{
					...
					{
						var link = cj('<a href="javascript:" class="smarty-suggestion">' + json.suggestions[j].text.replace(/<|>/g, "") + '</a>');
			...
			cj.getJSON("https://autocomplete-api.smartystreets.com/suggest?callback=?", {
				"auth-id": config.key,
				prefix: data.input,
			...
			{
				trigger("AutocompleteReceived", cj.extend(data, {
					json: json,
		...
		function useAutocompleteSuggestion(addr, suggestion, containerUi)
		{
			...
			if (addr.isFreeform())
				cj(domfields['street']).val(suggestion.text).change();
			else
			{
				if (domfields['street'])
					cj(domfields['street']).val(suggestion.street_line).change();
				if (domfields['city'])
					cj(domfields['city']).val(suggestion.city).change();
				if (domfields['state'])
					cj(domfields['state']).val(suggestion.state).change();
				if (domfields['lastline'])
					cj(domfields['lastline']).val(suggestion.city + " " + suggestion.state).change();
			}
		}
		...
		function filterDomElement(domElement, names, labels)
		{
			...
			var selectorSafeID = id.replace(/[\[|\]|\(|\)|\:|\'|\"|\=|\||\#|\.|\!|\||\@|\^|\&|\*]/g, '\\\\cj&');
			...
			{
				var lbl = cj('label[for="' + selectorSafeID + '"]')[0] || cj(domElement).parents('label')[0];
			...
			for (var i = 0; i < domElement.labels.length; i++)
			{
				...
				for (var j = 0; j < labels.length; j++)
					if (cj(domElement.labels[i]).text().toLowerCase().indexOf(labels[j]) > -1)
						return true;
			}
		...
		function userAborted(uiPopup, e)
		{
			...
			cj(document).unbind('keyup');
			cj(uiPopup).slideUp(defaults.speed, function() { cj(this).parent('.smarty-ui').remove(); });
		...
		function undelegateAllClicks(selectors)
		{
			for (var selector in selectors)
				cj('body').undelegate(selectors[selector], 'click');
		}
		...
		this.clean = function()
		{
			...
			for (var i = 0; i < forms.length; i++)
			{
				cj(forms[i].dom).data(mapMeta.formDataProperty, '');
				...
				for (var j = 0; j < forms[i].addresses.length; j++)
				{
					...
					{
						if (config.debug)
							cj(doms[prop]).css('background', 'none').attr('placeholder', '');
						cj(doms[prop]).unbind('change');
					}
					if (doms['street'])
						cj(doms['street']).unbind('keyup').unbind('keydown').unbind('blur');
				}

				// Unbind our form submit and submit-button click handlers
				cj.each(forms, function(idx) { cj(this.dom).unbind('submit', submitHandler); });
				cj(config.submitSelector, forms[i].dom).each(function(idx) { cj(this).unbind('click', submitHandler); });
			}

			cj('.smarty-ui').undelegate('.smarty-suggestion', 'click').undelegate('.smarty-suggestion', 'mouseover').undelegate('.smarty-suggestion', 'mouseleave').remove();
			cj('body').undelegate('.smarty-undo', 'click');
			cj('body').undelegate('.smarty-tag-grayed', 'click');
			cj(window).unbind('resize');
			cj(document).unbind('keyup');
		...
		this.automap = function(context)
		{
			...
			//cj('form').add(cj('iframe').contents().find('form')).each(function(idx) 	// Include forms in iframes, but they must be hosted on same domain (and iframe must have already loaded)
			cj('form').each(function(idx)	 // For each form ...
			{
				...
				for (var fieldName in mapMeta.identifiers)
				{
					...
					potential[fieldName] = cj(config.fieldSelector, this)
						.filter(function()
						{
							// This potential address input element must be within the user's set of selected elements
							return cj(context).has(this).length > 0; // (Using has() is compatible with as low as jQuery 1.4)
						})
		...
		this.mapFields = function(map, context)
		{
			...
			// Context can be acquired like: cj('#something').not('#something-else').LiveAddress( ... ); ...
			...
			for (var addrIdx in map)
			{
				...
				for (var fieldType in address)
				{
					if (fieldType != "id")
					{
						...
						var matched = cj(address[fieldType], context);
						if (matched.length == 0)
		...
		this.disableFields = function(address)
		{
			...
			for (var field in fields)
				cj(fields[field]).prop ? cj(fields[field]).prop('disabled', true) :  cj(fields[field]).attr('disabled', 'disabled');

			// Disable submit buttons
			if (address.form && address.form.dom)
			{
				var buttons = cj(config.submitSelector, address.form.dom);
		...
		this.enableFields = function(address)
		{
			...
			var fields = address.getDomFields();
			for (var field in fields)
				cj(fields[field]).prop ? cj(fields[field]).prop('disabled', false) : cj(fields[field]).removeAttr('disabled');
		...
		this.showLoader = function(addr)
		{
			...
			var loaderUI = cj('.smarty-dots.smarty-addr-'+addr.id()).parent();
			...
			cj('.smarty-dots', loaderUI).show();
		};

		this.hideLoader = function(addr)
		{
			if (config.ui)
				cj('.smarty-dots.smarty-addr-'+addr.id()).hide();
		};
		...
		this.markAsValid = function(addr)
		{
			...
			var domTag = cj('.smarty-tag.smarty-tag-grayed.smarty-addr-'+addr.id());
			domTag.removeClass('smarty-tag-grayed').addClass('smarty-tag-green').attr("title", "Address verified! Click to undo.");
			cj('.smarty-tag-text', domTag).text('Verified').hover(function () {
				cj(this).text('Undo');
			}, function() {
				cj(this).text('Verified');
		...
		this.unmarkAsValid = function(addr)
		{
			...
			if (!config.ui || !addr || cj(validSelector).length == 0)
				return;

			var domTag = cj('.smarty-tag.smarty-tag-green.smarty-addr-'+addr.id());
			domTag.removeClass('smarty-tag-green').addClass('smarty-tag-grayed').attr("title", "Address not verified. Click to verify.");
			cj('.smarty-tag-text', domTag).text('Verify').unbind('mouseenter mouseleave').removeClass('smarty-undo');
		};
		...
		this.showAmbiguous = function(data)
		{
			...
			html += '</div></div></div>';
			cj(html).hide().appendTo('body').show(defaults.speed);

			// Scroll to it if needed
			if (cj(document).scrollTop() > corners.top - 100
				|| cj(document).scrollTop() < corners.top - cj(window).height() + 100)
			{
				cj('html, body').stop().animate({
					scrollTop: cj('.smarty-popup.smarty-addr-'+addr.id()).offset().top - 100
				}, 500);
			}
			...
			cj('body').delegate(data.selectors.goodAddr, 'click', data, function(e)
			{
				cj('.smarty-popup.smarty-addr-'+addr.id()).slideUp(defaults.speed, function()
				{
					cj(this).parent('.smarty-ui').remove();
					cj(this).remove();
				});
				...
				trigger("UsedSuggestedAddress", {
					...
					chosenCandidate: response.raw[cj(this).data('index')]
				});
			});

			// User wants to revert to what they typed (forced accept)
			cj('body').delegate(data.selectors.useOriginal, 'click', data, function(e)
			{
				cj(this).parents('.smarty-popup').slideUp(defaults.speed, function()
				{
					cj(this).parent('.smarty-ui').remove();
					cj(this).remove();
				});
			...
			cj(document).keyup(data, function(e)
			{
				if (e.keyCode == 27) //Esc
				{
					undelegateAllClicks(e.data.selectors);
					delete e.data.selectors;
					userAborted(cj('.smarty-popup.smarty-addr-'+e.data.address.id()), e);
					suppress(e);
				}
			});

			// User clicks "x" in corner or chooses to try a different address (same effect as Esc key)
			cj('body').delegate(data.selectors.abort, 'click', data, function(e)
			{
				undelegateAllClicks(e.data.selectors);
				delete e.data.selectors;
				userAborted(cj(this).parents('.smarty-popup'), e);
			});
		};
	...
	function Address(domMap, formObj, addressID)
	{
		...
		var doSet = function(key, value, updateDomElement, keepState, sourceEvent, suppressAutoVerify)
		{
			...
			if (differentVal && !keepState)
			{
				...
				var uiTag = config.ui ? cj('.smarty-ui .smarty-tag.smarty-addr-'+id) : undefined;
				if (self.isDomestic())
				{
		...
		this.load = function(domMap, addressID)
		{
			...
			if (typeof domMap === 'object')	// can be an actual map to DOM elements or just field/value data
			{
				...
				for (var prop in domMap)
				{
					...
					!isData && cj(domMap[prop]).change(data, function(e)
					{
		...
		this.corners = function(lastField)
		{
			...
			if (!lastField)
			{
				...
				{
					if (!fields[prop].dom || !cj(fields[prop].dom).is(':visible'))
						continue;

					var dom = fields[prop].dom;
					var offset = cj(dom).offset();
					offset.right = offset.left + cj(dom).outerWidth();
					offset.bottom = offset.top + cj(dom).outerHeight();
		...
		this.toRequest = function()
		{
			...
			for (var key in fields)
			{
				...
				cj.extend(obj, keyval);
			}
			return cj.extend(obj, {candidates: config.candidates});
		};
		...
		{
			return config.ui && config.autocomplete && cj('.smarty-autocomplete.smarty-addr-'+self.id()).is(':visible');
		}
	...
	*/
	...
	var submitForm = function(invokeOn, invokeFunction)
	{
		if (invokeOn && typeof invokeOn !== 'function' && invokeFunction)
			if (invokeFunction == "click")
				cj(invokeOn).trigger('click');	// Very particular: we MUST fire the native 'click' event!
			else if (invokeFunction == "submit")
				cj(invokeOn).submit();	// For submit(), we have to use jQuery's, so that all its submit handlers fire.
	};
	...
	var EventHandlers = {
		FieldsMapped: function(event, data)
		{
			...
			window.loaded ? ui.postMappingOperations() : cj(window).load(ui.postMappingOperations);
		},
	...
	function trigger(eventType, metadata)
	{
		...
		cj(document).triggerHandler(eventType, metadata);
	}

	function bind(eventType)
	{
		// Bind a custom handler to an event
		cj(document).bind(eventType, HandleEvent);
	}
